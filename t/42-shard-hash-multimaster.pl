...
my @incline_cmd = (
    qw(src/incline),
    "--rdbms=$ENV{TEST_DBMS}",
    qw(--mode=shard --source=example/shard-multimaster-hash.json),
    qw(--database=test),
);
...
{ # check that changes are immediately applied on a single server
    ...
    ok(
        $dbh[0]->do(q{INSERT INTO incline_tweet (user_id,body) VALUES (4,'hello')}),
        'tweet',
    );
    is_deeply($cmpf->(), 'post tweet check');
    ok(
        $dbh[0]->do(q{INSERT INTO incline_tweet (user_id,body) VALUES (8,'ciao')}),
        'tweet 2',
    );
    is_deeply($cmpf->(), 'post tweet check 2');
    ok(
        $dbh[0]->do(q{INSERT INTO incline_tweet (user_id,body) VALUES (12,'hola')}),
        'tweet 3',
    );
    ...
    ok(
        $dbh[0]->do('DELETE FROM incline_tweet WHERE user_id=4'),
        'delete one tweet',
    );
...
{ # async tests
    ...
    ok(
        $dbh[0]->do(q{INSERT INTO incline_tweet (user_id,body) VALUES (4,'hello again')}),
        'tweet',
    );
    is_deeply($cmpf->(), 'post tweet check');
    ok(
